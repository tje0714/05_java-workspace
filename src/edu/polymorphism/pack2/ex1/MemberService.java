...
//    부모클래스[] 부모클래스변수이름    자식클래스1,자식클래스2,자식클래스3
        ...
        // 회원들은 향상된 for문을 활용해서 일반회원 부터 프리미엄회원 ,VIP 회원까지 순차적으로 개발자가 원하는 기능 실행
        for(Member   회원 : 회원들) { // 회원들 리스트에 작성된 모든 회원들을 회원 이라는 변수이름에 하나씩 넣어서 기능실행
            // 공통 메서드 사용
            회원.login();
            ...
            System.out.println("원가 : " + 물건원래가격 + "원"); // ctrl + space 클릭하면 추천 키워드 나 메서드 출력
            System.out.println("할인된 최종가격 : " + 할인된최종가격 + "원");

            // 일반회원에는 존재하지 않고, 프리미엄 과 VIP 회원에는 존재하는 변수이름 이 있음
        ...
        double 물건원래가격 = 10000;
        double 할인된최종가격 = 물건원래가격 * (1- 일반회원.할인율()); // 물건가에서 할인된 %만큼 빼고 최종가격나옴

        System.out.println("원가 : " + 물건원래가격 + "원");
        System.out.println("등급별 할인된 가격  : " + 할인된최종가격 + "원");
        ...
        double 물건원래가격 = 10000;
        double 할인된최종가격 = 물건원래가격 * (1- 프리미엄회원.할인율()); // 물건가에서 할인된 %만큼 빼고 최종가격나옴
        ...
        double 물건원래가격 = 10000;
        double 할인된최종가격 = 물건원래가격 * (1- VIP회원.할인율()); // 물건가에서 할인된 %만큼 빼고 최종가격나옴
