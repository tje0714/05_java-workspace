...
/**
 * 직원의 자식클래스 로 반드시 구현해야하는 기능들을 추가해서 문서 작성을 완료하도록 설정할 것
 *
 * extends 부모클래스    부모는 오직 하나!
 * implements 구현해야하는기능들, ... 2개 이상 작성 가능
 *
 * implements 는 반드시 extends 뒤에 와야하는 것이 아니라
 * 단독으로 사용 가능
 *
 * 상속을 받지 않아도 되는 문서에 필수적으로 작성해야하는 기능들이 존재할 때 사용
 */
public class 개발자 extends 직원 implements 업무수행기능들 {
    // class 개발자 implements 업무수행기능들 {} 로 상속이 필요하지 않는 경우 필히 구현해야하는 기능들에 대한 문서만 작성하기
    // 설정을 할 수 있음
