...
public class LoopEx3 {
    ...
    public void method1(){
        ...
        for (int i = 0; i < str.length(); i++) {
           ...
           /*
            str 은 문자열 이기 때문에 문장 전체에서 자리를 바라본다는 것 자체가
            문제
     ...
     * while 문이 false 여도 최초 1회는 무조건 조건이 true일 때
     * 식을 실행
     */
    /**
        0이 입력될 때 까지 정수를 계속 입력 받고 0 입력 시 누적된 합계 출력
     */
    public void method4(){
        int sum = 0;
        int input = 0;
        ...
        do { // 입력한 값이 0 이 아닐 때 반복
            ...
            input = sc.nextInt();
            sum += input;
        } while (input != 0); // input 이 0이 아닐 때 반복하기 위한 조건을 작성
    ...
    public void method5(){
        ...
        // index는 0 부터 시작하지만 length 는 1부터 시작
     ...
     * Math.random() * 10             // 0.0 <= 값  < 10.0  (0 이외 1 부터 모두 10까지로 범위 증가)
     ...
     *
     * 변수 이름 : String checkPassword = "java1234"  데이터 저장하는 변수이름
     *            String input =""  // 비밀번호 입력하고 입력된 비밀번호 데이터 저장하는 변수이름
     *            int attempts = 0; //시도횟수 데이터 저장하는 변수이름
     */
    public void method8(){
        ...
        int attempts = 0;
        do {
            System.out.print("시도횟수(" + attempts + "/5) 비밀번호를 입력하세요 : ");
            input = sc.next();
