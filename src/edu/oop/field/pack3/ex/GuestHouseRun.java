...
public class GuestHouseRun {
    public static void main(String[] args) {
        ...
        //  GuestHouse 라는 클래스 이름으로 호출하는 것이 맞으므로
        //  자동완성에서 누락되는 것이 맞음)
        ...
        System.out.println("guest1 이 보이는 총 방문자 수 : " + guest1.totalCount); // GuestHouse.totalCount
        // 29번 째 줄에서 실행한 방문자 수는
        // 게스트 1번 사람만 보이는 방문자 수가 아니라 GuestHouse 클래스에서 자체적으로
        // 모든 게스트 하우스 사람들에게 제공하는 숫자이므로
        // 35 번째 줄 에서 보이는 총 방문자 수는 guest1, guest2 모두 방문한 카운트가 보이는 것이 맞음
        ...
        guest1.guestInfo(); // 게스트 하우스가 공통으로 제공하는 것 아닌 방문자 개별로 알아야하 는 정보
        guest2.guestInfo(); // 게스트 하우스가 공통으로 제공하는 것 아닌 방문자 개별로 알아야하 는 정보
